function [x,n_iters,J_vals,snr_value,runtimes] = TwIST(y,A,lambda,varargin)
% *************************************************************************
...
switch init
    ...
    case 2   % initialize x = AT*y
       x = AT(y); 
    case 3   % initial x was given as a function argument; just check size
        if size(x) ~= size(AT(y))
...
while loop      % main loop
    ...
    snr_value(iter) = snrv(x,objective);
    ...
    % display progress
    if verbose
        fprintf(['iter: %5d | type: %s | cost: %10.4e | PSNR: %2.4e | stepsize: %2.2e | ', ...
            'criterion: %10.4e | runtime: %5.1f s\n'], ...
            iter, str, J_vals(iter+1),snr_value(iter), 1/L, crit, runtimes(iter));
    end
